...
class ResGCN(torch.nn.Module):
    ...
    def __init__(self, dataset=None, hidden=128, num_feat_layers=1, num_conv_layers=3,
                 ...
                 edge_norm=True, num_aug=None, classification=True):
...
class ResGCN_graphcl(ResGCN):
    def __init__(self, **kargs):
        ...
        hidden = kargs['hidden']
        self.proj_head = nn.ModuleList([nn.Sequential(nn.Linear(128, 128), nn.ReLU(inplace=True), nn.Linear(128, 128))
    ...
    def forward_graphcl(self, data):
        ...
        if self.use_xg:
            ...
            xg = None
        # print(f'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n'
        #       f'MODEL INPUT: x = {x}, edge_index = {edge_index}, batch = {batch}, xg = {xg}')
        return self.forward_BNConvReLU_graphcl(x, edge_index, batch, xg)

    def forward_BNConvReLU_graphcl(self, x, edge_index, batch, xg=None):
        print(f'INPUT len: {x.shape}')
        x = self.bn_feat(x)
        ...
        for i, conv in enumerate(self.convs):
            ...
            x_ = F.relu(conv(x_, edge_index))
            x = x + x_ if self.conv_residual else x_
            print(f'LAYER {tmp + i}: {x.shape}')
            tmp += 1

        gate = 1 if self.gating is None else self.gating(x)
        x = self.global_pool(x * gate, batch)
        print(f'LAYER {tmp}: {x.shape}')
        tmp += 1
