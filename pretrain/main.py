...
def run_experiment_graphcl(dataset_feat_net_triple
                           ...
                           lr_test=None, gamma_joao_test=None):
    ...
    if args.nni:
        ...
        lr = params['lr']
        gamma_joao = params['gamma_joao']
    else:
        lr = lr_test if lr_test else args.lr
        gamma_joao = gamma_joao_test if gamma_joao_test else args.gamma_joao

    if not args.model == 'joao':
        experiment_graphcl.experiment(dataset, model_func, epochs=args.epochs, batch_size=args.batch_size, lr=args.lr, weight_decay=0, dataset_name=dataset_name, aug_mode=args.aug_mode, aug_ratio=args.aug_ratio, suffix=args.suffix)

    else:
        loss = experiment_joao.experiment(dataset, model_func, epochs=args.epochs, batch_size=args.batch_size, lr=lr, weight_decay=0, dataset_name=dataset_name, aug_mode=args.aug_mode, aug_ratio=args.aug_ratio, suffix=args.suffix, gamma_joao=gamma_joao)
        if args.nni:
            nni.report_final_result(mean(loss))
        return loss
...
if __name__ == '__main__':
    ...
    for lr, gamma in product(lr_values, gamma_values):
        loss = run_experiment_graphcl(lr_test=lr, gamma_joao_test=gamma)
        with open('pretrain_tuning.txt', 'a') as f:
            f.write(f'dataset: {args.dataset}   lr: {lr}   gamma: {gamma}   loss: {mean(loss)}\n')

        results.append([args.dataset, lr, gamma] + loss)

    with open('pretrain_tuning_results.csv', 'w') as f:
        writer = csv.writer()
